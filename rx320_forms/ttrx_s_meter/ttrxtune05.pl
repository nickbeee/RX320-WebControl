...
sub TuneRx {
    ...
    my $AdjTfreq =
      $Tfreq - 1250 + ( int( $mcor * ( $Fcor + $pbt + $cwbias ) ) );
...
sub SendToRx {
    ...
    #flush buffers
    $RadioPort->lookclear;
	#delay to allow meter to settle
	sleep(1);
    ...
    #check data for validity and recalc.
    #(string starts with X)
    #If invalid, return 0
    #Meter range between 0 and 10000 (approx) for RX-320D

    if ( $meter =~ /^X/ ) {

        #$meter = substr($meter,1,2);
        my $meterhi = ord( substr( $meter, 1 ) );
        my $meterlo = ord( substr( $meter, 2 ) );
        $meter = $meterhi * 256 + $meterlo;
